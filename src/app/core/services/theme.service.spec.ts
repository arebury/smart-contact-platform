...
describe('ThemeService', () => {
  beforeEach(() => {
    localStorage.clear();
    document.documentElement.classList.remove('aed-dark');
  ...
  describe('DOM application', () => {
    it('adds the .aed-dark class on <html> when dark', () => {
      const svc = TestBed.inject(ThemeService);
      svc.set('dark');
      TestBed.flushEffects();
      expect(document.documentElement.classList.contains('aed-dark')).toBe(true);
    });

    it('removes the .aed-dark class on <html> when switching to light', () => {
      const svc = TestBed.inject(ThemeService);
      svc.set('dark');
      TestBed.flushEffects();
      svc.set('light');
      TestBed.flushEffects();
      expect(document.documentElement.classList.contains('aed-dark')).toBe(false);
    });
  });
});

...
export class ClasificacionIAStore implements RepoStore<ClasificacionIA> {
  ...
  addItem(data: Omit<ClasificacionIA, 'id'>): ClasificacionIA { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<ClasificacionIA>): void { this.store.updateItem(id, updates); }
  deleteItem(id: number): void { this.store.deleteItem(id); }
  deleteItems(ids: readonly number[]): void { this.store.deleteItems(ids); }
}

const COLUMNS: readonly RepoColumnDef<ClasificacionIA>[] = [
  { key: 'name', labelKey: 'repositories.columns.name', kind: 'text', accessor: (i) => i.name, width: '224px', emphasis: true },
  { key: 'model', labelKey: 'repositories.clasificacion_ia.model', kind: 'text', accessor: (i) => i.model, width: '128px' },
  { key: 'categories', labelKey: 'repositories.clasificacion_ia.categories', kind: 'truncate', accessor: (i) => i.categories },
  { key: 'accuracy', labelKey: 'repositories.clasificacion_ia.accuracy', kind: 'mono', accessor: (i) => i.accuracy, width: '96px' },
  {
    key: 'status', labelKey: 'repositories.columns.status', kind: 'status', width: '96px',
    accessor: (i) => i.status,
    statusMap: {
...
const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.clasificacion_ia.name_placeholder' },
  { key: 'model', labelKey: 'repositories.clasificacion_ia.model', type: 'text', required: true, placeholderKey: 'repositories.clasificacion_ia.model_placeholder' },
  { key: 'categories', labelKey: 'repositories.clasificacion_ia.categories', type: 'textarea', required: true, placeholderKey: 'repositories.clasificacion_ia.categories_placeholder' },
  { key: 'accuracy', labelKey: 'repositories.clasificacion_ia.accuracy', type: 'text', placeholderKey: 'repositories.clasificacion_ia.accuracy_placeholder' },
  { key: 'description', labelKey: 'repositories.columns.description', type: 'textarea', placeholderKey: 'repositories.placeholders.description' },
  {
    key: 'status', labelKey: 'repositories.columns.status', type: 'select',
    options: [
...
@Component({
  selector: 'aed-clasificacion-ia-page',
  standalone: true,

...
export class VariablesStore implements RepoStore<RepoVariable> {
  ...
  addItem(data: Omit<RepoVariable, 'id'>): RepoVariable { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<RepoVariable>): void { this.store.updateItem(id, updates); }
...
const COLUMNS: readonly RepoColumnDef<RepoVariable>[] = [
  ...
  { key: 'key', labelKey: 'repositories.variables.key', kind: 'mono', accessor: (i) => i.key, width: '128px' },
  { key: 'defaultValue', labelKey: 'repositories.variables.default', kind: 'text', accessor: (i) => i.defaultValue, width: '160px' },
  { key: 'type', labelKey: 'repositories.variables.type', kind: 'text', accessor: (i) => i.type, width: '96px' },
];

const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.variables.name_placeholder' },
  { key: 'key', labelKey: 'repositories.variables.key', type: 'text', required: true, placeholderKey: 'repositories.variables.key_placeholder' },
  { key: 'defaultValue', labelKey: 'repositories.variables.default', type: 'text', placeholderKey: 'repositories.variables.default_placeholder' },
  {
    key: 'type', labelKey: 'repositories.variables.type', type: 'select',
    options: [

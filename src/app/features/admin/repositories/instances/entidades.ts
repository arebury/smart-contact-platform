...
export class EntidadesStore implements RepoStore<Entidad> {
  private readonly store: LocalStore<Entidad> = createLocalStore<Entidad>({
    storageKey: 'smartcontact_entidades_repo',
    versionKey: 'smartcontact_entidades_repo_v',
    currentVersion: 1,
    defaults: SEED,
  });
  readonly items = this.store.items;
  addItem(data: Omit<Entidad, 'id'>): Entidad { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<Entidad>): void { this.store.updateItem(id, updates); }
...
const COLUMNS: readonly RepoColumnDef<Entidad>[] = [
  ...
  { key: 'type', labelKey: 'repositories.entidades.type', kind: 'text', accessor: (i) => i.type, width: '112px' },
  { key: 'values', labelKey: 'repositories.entidades.values', kind: 'truncate', accessor: (i) => i.values },
];

const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.entidades.name_placeholder' },
  {
    key: 'type', labelKey: 'repositories.entidades.type', type: 'select',
    options: [
  ...
  { key: 'values', labelKey: 'repositories.entidades.values', type: 'textarea', placeholderKey: 'repositories.entidades.values_placeholder' },

...
export class AgendasStore implements RepoStore<Agenda> {
  ...
  addItem(data: Omit<Agenda, 'id'>): Agenda { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<Agenda>): void { this.store.updateItem(id, updates); }
...
const COLUMNS: readonly RepoColumnDef<Agenda>[] = [
  ...
  { key: 'numbers', labelKey: 'repositories.agendas.numbers', kind: 'truncate', accessor: (i) => i.numbers },
  {
...
const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.agendas.name_placeholder' },
  { key: 'numbers', labelKey: 'repositories.agendas.numbers', type: 'textarea', required: true, placeholderKey: 'repositories.agendas.numbers_placeholder' },

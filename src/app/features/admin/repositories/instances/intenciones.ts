...
export class IntencionesStore implements RepoStore<Intencion> {
  ...
  addItem(data: Omit<Intencion, 'id'>): Intencion { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<Intencion>): void { this.store.updateItem(id, updates); }
...
const COLUMNS: readonly RepoColumnDef<Intencion>[] = [
  ...
  { key: 'category', labelKey: 'repositories.intenciones.category', kind: 'text', accessor: (i) => i.category, width: '128px' },
  { key: 'examples', labelKey: 'repositories.intenciones.examples', kind: 'truncate', accessor: (i) => i.examples },
];

const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.intenciones.name_placeholder' },
  { key: 'category', labelKey: 'repositories.intenciones.category', type: 'text', required: true, placeholderKey: 'repositories.intenciones.category_placeholder' },
  { key: 'examples', labelKey: 'repositories.intenciones.examples', type: 'textarea', required: true, placeholderKey: 'repositories.intenciones.examples_placeholder' },

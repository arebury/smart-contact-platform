...
export class TipificacionesStore implements RepoStore<Tipificacion> {
  ...
  addItem(data: Omit<Tipificacion, 'id'>): Tipificacion { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<Tipificacion>): void { this.store.updateItem(id, updates); }
...
const COLUMNS: readonly RepoColumnDef<Tipificacion>[] = [
  ...
  { key: 'code', labelKey: 'repositories.tipificaciones.code', kind: 'mono', accessor: (i) => i.code, width: '96px' },
  { key: 'category', labelKey: 'repositories.tipificaciones.category', kind: 'text', accessor: (i) => i.category, width: '128px' },
  { key: 'description', labelKey: 'repositories.columns.description', kind: 'truncate', accessor: (i) => i.description },
];

const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.tipificaciones.name_placeholder' },
  { key: 'code', labelKey: 'repositories.tipificaciones.code', type: 'text', required: true, placeholderKey: 'repositories.tipificaciones.code_placeholder' },
  { key: 'category', labelKey: 'repositories.tipificaciones.category', type: 'text', required: true, placeholderKey: 'repositories.tipificaciones.category_placeholder' },

...
export class ReglasIAStore implements RepoStore<ReglaIA> {
  ...
  addItem(data: Omit<ReglaIA, 'id'>): ReglaIA { return this.store.addItem(data); }
  updateItem(id: number, updates: Partial<ReglaIA>): void { this.store.updateItem(id, updates); }
...
const COLUMNS: readonly RepoColumnDef<ReglaIA>[] = [
  ...
  { key: 'condition', labelKey: 'repositories.reglas_ia.condition', kind: 'truncate', accessor: (i) => i.condition },
  { key: 'action', labelKey: 'repositories.reglas_ia.action', kind: 'truncate', accessor: (i) => i.action },
  {
    key: 'priority', labelKey: 'repositories.reglas_ia.priority', kind: 'status', width: '96px',
...
const FIELDS: readonly RepoFieldDef[] = [
  { key: 'name', labelKey: 'repositories.columns.name', type: 'text', required: true, placeholderKey: 'repositories.reglas_ia.name_placeholder' },
  { key: 'condition', labelKey: 'repositories.reglas_ia.condition', type: 'textarea', required: true, placeholderKey: 'repositories.reglas_ia.condition_placeholder' },
  { key: 'action', labelKey: 'repositories.reglas_ia.action', type: 'textarea', required: true, placeholderKey: 'repositories.reglas_ia.action_placeholder' },
  {
    key: 'priority', labelKey: 'repositories.reglas_ia.priority', type: 'select',
    options: [

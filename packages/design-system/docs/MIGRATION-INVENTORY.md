...
| 35 | Divider | ✓ | 🟢 Extended | `<sc-divider>` | [302:11810](https://www.figma.com/design/khNq9dJKNi13pNllrqm6dx/Smart-Contact-Prime?node-id=302-11810) | **100%** — **S69 cocinado** wrapper de `<p-divider>` (era `<hr class="divider">`). API `[layout]`/`[type]`/`[align]` + `<ng-content>` → cubre todas las variantes del Kit. Tokens en `sc-preset.ts divider.*` (margin 14 / content-padding 7 escala 14-base vs 16/8 Aura; borde gray-200; content `--sc-text-secondary`/`--sc-bg-surface`). 10 valores de sizing en `tokens:parity §4` (1:1 export). Consumer: config Contact Center (horizontal-solid). Mapping Kit (ejes Type/Content/Align/Direction, tokens, margins) en `code-connect-mapping.md §Divider`. | `code-connect-mapping.md §Divider` |

...
| 33 | Deuda CSS | `.page` + `.page__inner` chrome list-pages Memory | Patrón `<div class="page"><div class="page__inner">` duplicado en 4 pages Memory + 4 AED list-pages = 8 copias scoped del mismo chrome (`max-width 1600` + `padding spacing-500/600` + `display flex column gap`). S42 fix puntual: añadido a las 4 pages Memory que solo lo declaraban en HTML (sin estilos → tabla full-width). Considerar extraer a partial `_sc-page-chrome.scss` cuando aparezca 9º consumer o cuando Memory cierre todas sus pages. **Mismo principio que #32**. | P3 | Futura (≥9º consumer) | 🆕 Detectada | S42 |

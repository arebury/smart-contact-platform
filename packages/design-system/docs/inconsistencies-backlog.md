...
| 35 | UX delta | `<sc-memory-conversation-table>` row-click semantic | Audit S44: click en row de Memory conversations dispara `selectionToggled.emit()` (marca checkbox); solo click en `<sc-memory-status-icon>` abre el player. AED list-pages tienen click-en-row → form-edit. Usuario que viene de AED espera consistent affordance row→primary-action. Decisión pendiente Rafa: mantener selección-first (Memory dominante flow), alinear a AED edit-first, o doble-click→player + click→select. Entry detallada en `docs/memory-migration-inventory.md §10 #19`. | P2 | Decisión Rafa | 🆕 Detectada | S44 |

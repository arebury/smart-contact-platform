...
| 34 | Inconsistencia visual | `delete-entity-dialog` HTML markup | Audit S42 detectó: AED delete-user-modal muestra botón Eliminar GRIS pese a que `delete-entity-dialog.component.html:85` aplica `class="btn btn--danger"`; AED delete-label-modal (componente propio `delete-labels-dialog`) muestra Eliminar ROJO porque usa `<p-button severity="danger">` correcto. Causa: el HTML `delete-entity-dialog` (y galleries `bulk-action-bar` / `bulk-edit-menu` docs) sigue usando `.btn--danger` huérfano tras refactor `.btn` → `<p-button>` S34 (#11). Acción: migrar `delete-entity-dialog.component.html` a `<p-button severity="danger">` + sweep galleries doc. | P2 | Próxima sesión | 🆕 Detectada | S42 |

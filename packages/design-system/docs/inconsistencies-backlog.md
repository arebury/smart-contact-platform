...
| 31 | Build / bundle | AED initial chunk | Bundle initial 1.42 MB → 1.62 MB (+200 KB) tras introducir primer uso real de `<sc-multi-select>` y `<sc-datepicker>` en Memory ConversationsView iter 3 (S37). **S39 investigación**: confirmado que ambos están en chunks iniciales — `chunk-5VMR26S7.js` (383 KB) contiene `p-multiselect` (89 hits) + `p-select` + `p-button` + `p-toggleswitch` etc; `chunk-YEAYSKUX.js` (357 KB) contiene `p-datepicker` (214 hits) + `p-popover` + `p-dialog` (calendar popup). Causa raíz **descartada**: NO es por imports del barrel (`@shared/components`) — los wrappers Memory usan paths granulares (`@shared/components/multi-select/multi-select.component`). Causa raíz **probable**: Angular common chunk extraction — MultiSelect y DatePicker se usan en 2 lazy components Memory (conversation-filters + rule-builder-page) → Angular los promueve a chunk compartido que se carga eager con el shell. Es el comportamiento por defecto cuando un módulo se importa en ≥2 lazy entries. **Próximos pasos**: (a) opción A: extraer rule-builder-page MultiSelect a dynamic `import()` para volver a 1-consumer y que el chunk caiga a lazy; (b) opción B: aceptar el bundle como está (gzip transfer 363 KB es razonable; budget 1.8 MB documentado). **Recomendación S39**: mantener diferido hasta que Memory tenga primer rollout real — entonces medir Web Vitals (LCP, FID) y decidir si el bump merece refactor. Initial 1.65 MB raw ≈ 363 KB transferred (gzip ratio 0.22). | P3 | Futura | ⏸️ Diferida con investigación S39 | S37 |

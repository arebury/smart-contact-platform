...
| 5 | Gap componente | SCDS | Falta `<sc-input-group>` (Figma SC node 6738:22644). Wrapper de `<p-inputgroup>` para addon left/right. Bloquea migración tag-input en aed-servicio. | P2 | Futura | 🆕 Detectada | S31 |
...
| 9 | Deuda CSS | `sticky-form-header` | 8 `::ng-deep` redimensionando `<sc-photo-upload>` proyectado en slot. Debería usar el prop `[size]` ya existente en photo-upload. | P2 | Post audit tokens | 🆕 Detectada | S30 (00-diagnosis.md Fase 4) |
...
| 19 | Defensa CVA + signals | `input`, `select`, `multi-select`, `datepicker`, `input-number`, `search` | 6 wrappers escriben signals dentro de `writeValue()` sin `untracked()`. NG0600 solo se dispara si writeValue se llama desde computed/effect (no es el flujo Angular CVA normal), pero envolver con `untracked()` es defensa recomendada por Angular docs para CVA + signals. Sin evidencia de bug actual en producción. | P2 | Futura | 🆕 Detectada | S32 (Perplexity audit) |
| 20 | Figma drift detection | `customs-catalog.md` | Añadir columna `figma-last-verified` con fecha en cada entry. El check periódico se haría al cierre de cada sesión que toque Figma. Detecta drift entre Figma y código sin overhead de automatización. Patrón industria (Atlassian, IBM). | P3 | Futura | 🆕 Detectada | S32 (Perplexity audit) |
| 21 | Status low-usage en spec docs | `docs/components/*.md` | Patrón GitHub Primer: añadir cabecera `status: stable / experimental / low-usage / internal` para que consumers sepan qué componentes están maduros vs niche (ej. `command-palette`, `color-dot-picker`, `confirm-host` con 1 use podrían marcar `low-usage`). Reduce overhead percibido sin sacrificar cobertura. | P3 | Futura | 🆕 Detectada | S32 (Perplexity audit) |

...
| 26 | Frontmatter status en spec docs | `docs/components/*.md` | Pattern Carbon/Polaris: además del Lifecycle section centralizado (S32), añadir `> **Status**: stable · **AED uses**: N` inline en cada spec doc. Projection (no duplicación) — el consumer ve el status sin salir del doc. Lifecycle section sigue siendo source of truth. 33 docs editar. | P3 | Próxima sesión | 🆕 Detectada | S32 (Perplexity audit) |

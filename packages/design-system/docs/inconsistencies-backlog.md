...
| 31 | Build / bundle | AED initial chunk | Bundle initial 1.42 MB → 1.62 MB (+200 KB) tras introducir primer uso real de `<sc-multi-select>` y `<sc-datepicker>` en Memory ConversationsView iter 3 (S37). Hipótesis: los `MultiSelectModule` y `DatePickerModule` de PrimeNG no son tree-shakeable por su `sideEffects` y entran al initial chunk aunque Memory es lazy. Budget bumpeado de 1.5 MB → 1.8 MB error como mitigación inmediata. **Próximos pasos**: (a) verificar con `source-map-explorer` qué módulos exactos se promovieron al initial; (b) si la causa es PrimeNG side-effects, considerar dynamic `import()` dentro del chunk Memory para forzar code-splitting; (c) si no hay solución limpia, dejar el budget en 1.8 MB documentado. | P3 | Futura | 🆕 Detectada | S37 |

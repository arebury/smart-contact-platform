...
| 14 | **Templates predefinidos en CategoryFormModal** | `CreateCategoryPanel.tsx` React tiene Dialog secundario con 6 plantillas predefinidas (queja facturación, soporte técnico, etc.). Iter 11b: sin templates. | trigger Marta/Rafa | Si onboarding requiere seed rápido de categorías comunes. Lista ya está mockeada en `categories-mock.ts` — replicable como Dialog "Plantillas". |

...
| 21 | **Bug `<sc-memory-mock-sample-switcher>` popover** | S45 Rafa reportó 2 bugs visuales del popover de samples (11 entries post-S42): (a) al scrollear la lista, el popover **se corta abajo sin scroll interno** — opciones que no caben quedan invisibles; (b) cuando el usuario vuelve a scrollear la página hacia arriba, **el popover queda detrás del `sc-topbar`** (z-index issue o appendTo body que pierde posicionamiento). Probable causa: PopoverModule de PrimeNG con `appendTo="body"` + max-height no definido + z-index inferior al topbar sticky. Acción: añadir `[style]="{'max-height': '70vh', 'overflow-y': 'auto'}"` al panel + revisar z-index del `--sc-z-popover` token vs `--sc-z-sidebar`/`--sc-z-topbar`. Prototype-only, purga pre-deploy, baja urgencia comercial pero alto fastidio en demo. | Próxima ronda audit | Bug visual demo |

...
| 10 | **Enlace "Ver repositorio" en RuleBuilder** (3 dimensiones del Alcance: Servicios / Grupos ACD / Agentes) | Spec `rule-constructor-update.md §EVIDENCIAR EL ORIGEN`. Iter 9c-1: `href="javascript:void(0)"` sin acción. | post-vistas: HUB AED extendido | Cuando se extienda el `RepositoriosHubPageComponent` AED con cards IA Memory (decisión B §8). Mapear cada dimensión a su sub-ruta correspondiente del HUB AED: Servicios → ruta servicios AED, Grupos → /admin/groups, Agentes → /admin/agents. |

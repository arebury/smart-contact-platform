...
2. **Tipografía → VALIDAR EN REAL (DD-13 ya decidido).** Migration-safe confirmado (solo tocamos `--sc-*`/`app/*`/preset, nunca PrimeNG ni el Kit del equipo). Pasos: **(a)** rename variables typography a **barra** (`typography/font/size/X` + `typography/line/height/X`) — mecánico, hecho en el duplicado S72b; **(b)** crear variables de la rampa de **títulos**/contenido + atar text styles (hoy títulos = style sin variable → no cruzan al código); **(c)** repetir en el **Kit oficial** (el que lee el Theme Designer); **(d)** Theme Designer → PR a GitHub → verificar valores; **(e)** reflejar en código `--sc-font-size-*` en **rem** (no px — backlog #88) + redondo + ajustar `tokens:type-parity` + diff visual + e2e.
